...
const STRINGS = {
  en: {
    ...
    // why-sentences: the same numbers that chose the move, said in cards
    whyDead: (p) => `${p.card} can't be part of any combination any more.`,
    whyWeak: (p) => `${p.card} is worth ${p.max} at most from here.`,
    whyKeepReady: (p) => `${p.cards} already make ${p.score}.`,
    whyKeep: (p) => `${p.cards} stay: with ${p.missing} that's ${p.score}.`,
    whyAltPoints: (p) => `Throwing ${p.card} instead costs about ${p.points} points.`,
    whyAltChest: (p) => `Throwing ${p.card} instead drops your silver chance to ${p.silver}.`,
  ...
  de: {
    ...
    whyDead: (p) => `${p.card} kann keine Kombination mehr bilden.`,
    whyWeak: (p) => `${p.card} bringt von hier aus höchstens ${p.max}.`,
    whyKeepReady: (p) => `${p.cards} sind schon ${p.score}.`,
    whyKeep: (p) => `${p.cards} bleiben: mit ${p.missing} sind das ${p.score}.`,
    whyAltPoints: (p) => `Stattdessen ${p.card} zu werfen kostet rund ${p.points} Punkte.`,
    whyAltChest: (p) => `Stattdessen ${p.card} zu werfen drückt die Silberchance auf ${p.silver}.`,
  ...
  tr: {
    ...
    whyDead: (p) => `${p.card} artık hiçbir kombinasyona giremez.`,
    whyWeak: (p) => `${p.card} buradan en fazla ${p.max} eder.`,
    whyKeepReady: (p) => `${p.cards} zaten ${p.score} ediyor.`,
    whyKeep: (p) => `${p.cards} kalıyor: ${p.missing} ile ${p.score} olur.`,
    whyAltPoints: (p) => `Onun yerine ${p.card} atmak yaklaşık ${p.points} puana mal olur.`,
    whyAltChest: (p) => `Onun yerine ${p.card} atmak gümüş şansını ${p.silver} seviyesine düşürür.`,
  ...
  ro: {
    ...
    whyDead: (p) => `${p.card} nu mai poate intra în nicio combinație.`,
    whyWeak: (p) => `${p.card} valorează cel mult ${p.max} de aici.`,
    whyKeepReady: (p) => `${p.cards} fac deja ${p.score}.`,
    whyKeep: (p) => `${p.cards} rămân: cu ${p.missing} fac ${p.score}.`,
    whyAltPoints: (p) => `Dacă arunci ${p.card} în schimb, pierzi cam ${p.points} puncte.`,
    whyAltChest: (p) => `Dacă arunci ${p.card} în schimb, șansa de argint scade la ${p.silver}.`,
  ...
  es: {
    ...
    whyDead: (p) => `${p.card} ya no puede formar ninguna combinación.`,
    whyWeak: (p) => `${p.card} vale como mucho ${p.max} desde aquí.`,
    whyKeepReady: (p) => `${p.cards} ya suman ${p.score}.`,
    whyKeep: (p) => `${p.cards} se quedan: con ${p.missing} son ${p.score}.`,
    whyAltPoints: (p) => `Descartar ${p.card} en su lugar cuesta unos ${p.points} puntos.`,
    whyAltChest: (p) => `Descartar ${p.card} en su lugar baja la opción de plata al ${p.silver}.`,
  ...
  pl: {
    ...
    whyDead: (p) => `${p.card} nie utworzy już żadnego układu.`,
    whyWeak: (p) => `${p.card} da stąd najwyżej ${p.max}.`,
    whyKeepReady: (p) => `${p.cards} to już ${p.score}.`,
    whyKeep: (p) => `${p.cards} zostają: z ${p.missing} to ${p.score}.`,
    whyAltPoints: (p) => `Wyrzucenie ${p.card} zamiast tego kosztuje około ${p.points} punktów.`,
    whyAltChest: (p) => `Wyrzucenie ${p.card} zamiast tego obniża szansę na srebro do ${p.silver}.`,
