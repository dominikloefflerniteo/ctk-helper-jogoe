...
const STRINGS = {
  en: {
    ...
    session: "Session",
    scoreCeilingFull: (p) => `now ${p.floor} · +${p.best} if you confirm best`,
    scoreCeilingEmpty: (p) => `now ${p.floor}`,
    chestLabelGold: (p) => `Gold (${p.score} ≥ 400)`,
    chestLabelSilverLocked: (p) => `Silver locked (${p.score})`,
    chestLabelSilver: (p) => `Silver (${p.score})`,
    chestLabelBronze: (p) => `Bronze (${p.score})`,
    pts: "pts",
     games: "Games", avgScore: "Avg score", reset: "Reset",
    ...
    calculatingSuggestion: "Calculating best move…",
    noMovesPossible: "No moves left — the run is over.",
    ...
    cardInPlay: "That card is already in play.",
    deckEmptyNoThrow: "Can't throw — the deck is empty, nothing would replace it.",
    pickScoresNothing: "Those 3 cards score nothing — not taken.",
  ...
  de: {
    ...
    session: "Sitzung",
    scoreCeilingFull: (p) => `jetzt ${p.floor} · +${p.best} wenn du bestätigst`,
    scoreCeilingEmpty: (p) => `jetzt ${p.floor}`,
    chestLabelGold: (p) => `Gold (${p.score} ≥ 400)`,
    chestLabelSilverLocked: (p) => `Silber gesichert (${p.score})`,
    chestLabelSilver: (p) => `Silber (${p.score})`,
    chestLabelBronze: (p) => `Bronze (${p.score})`,
    pts: "Pkt",
     games: "Runden", avgScore: "Ø Punkte", reset: "Zurücksetzen",
    ...
    calculatingSuggestion: "Berechne besten Zug…",
    noMovesPossible: "Keine Züge mehr — das Spiel ist vorbei.",
    ...
    cardInPlay: "Diese Karte liegt bereits im Spiel.",
    deckEmptyNoThrow: "Kein Abwerfen — der Stapel ist leer, es käme nichts nach.",
    pickScoresNothing: "Diese 3 Karten geben 0 Punkte — nicht genommen.",
  ...
  tr: {
    ...
    session: "Oturum",
    scoreCeilingFull: (p) => `şimdi ${p.floor} · +${p.best} en iyiyi onaylarsan`,
    scoreCeilingEmpty: (p) => `şimdi ${p.floor}`,
    chestLabelGold: (p) => `Altın (${p.score} ≥ 400)`,
    chestLabelSilverLocked: (p) => `Gümüş kilitlendi (${p.score})`,
    chestLabelSilver: (p) => `Gümüş (${p.score})`,
    chestLabelBronze: (p) => `Bronz (${p.score})`,
    pts: "puan",
     games: "Tur", avgScore: "Ort. puan", reset: "Sıfırla",
    ...
    calculatingSuggestion: "En iyi hamle hesaplanıyor…",
    noMovesPossible: "Hamle kalmadı — oyun bitti.",
    ...
    cardInPlay: "Bu kart zaten oyunda.",
    deckEmptyNoThrow: "Atılamaz — deste boş, yerine kart gelmez.",
    pickScoresNothing: "Bu 3 kart puan getirmiyor — alınmadı.",
  ...
  ro: {
    ...
    session: "Sesiune",
    scoreCeilingFull: (p) => `acum ${p.floor} · +${p.best} dacă confirmi cel mai bun`,
    scoreCeilingEmpty: (p) => `acum ${p.floor}`,
    chestLabelGold: (p) => `Aur (${p.score} ≥ 400)`,
    chestLabelSilverLocked: (p) => `Argint asigurat (${p.score})`,
    chestLabelSilver: (p) => `Argint (${p.score})`,
    chestLabelBronze: (p) => `Bronz (${p.score})`,
    pts: "pct",
     games: "Runde", avgScore: "Punctaj mediu", reset: "Resetează",
    ...
    calculatingSuggestion: "Se calculează mutarea optimă…",
    noMovesPossible: "Nu mai sunt mutări — jocul s-a terminat.",
    ...
    cardInPlay: "Cartea este deja în joc.",
    deckEmptyNoThrow: "Nu poți arunca — pachetul e gol, nu vine nimic în loc.",
    pickScoresNothing: "Aceste 3 cărți nu aduc puncte — nu au fost luate.",
  ...
  es: {
    ...
    session: "Sesión",
    scoreCeilingFull: (p) => `ahora ${p.floor} · +${p.best} si confirmas lo mejor`,
    scoreCeilingEmpty: (p) => `ahora ${p.floor}`,
    chestLabelGold: (p) => `Oro (${p.score} ≥ 400)`,
    chestLabelSilverLocked: (p) => `Plata asegurada (${p.score})`,
    chestLabelSilver: (p) => `Plata (${p.score})`,
    chestLabelBronze: (p) => `Bronce (${p.score})`,
    pts: "pts",
     games: "Rondas", avgScore: "Puntos medios", reset: "Reiniciar",
    ...
    calculatingSuggestion: "Calculando el mejor movimiento…",
    noMovesPossible: "No quedan movimientos — el juego ha terminado.",
    ...
    cardInPlay: "Esa carta ya está en juego.",
    deckEmptyNoThrow: "No puedes descartar — el mazo está vacío, no habría reemplazo.",
    pickScoresNothing: "Esas 3 cartas no puntúan — no se han tomado.",
  ...
  pl: {
    ...
    session: "Sesja",
    scoreCeilingFull: (p) => `teraz ${p.floor} · +${p.best} jeśli potwierdzisz najlepsze`,
    scoreCeilingEmpty: (p) => `teraz ${p.floor}`,
    chestLabelGold: (p) => `Złota (${p.score} ≥ 400)`,
    chestLabelSilverLocked: (p) => `Srebrna zabezpieczona (${p.score})`,
    chestLabelSilver: (p) => `Srebrna (${p.score})`,
    chestLabelBronze: (p) => `Brązowa (${p.score})`,
    pts: "pkt",
     games: "Rundy", avgScore: "Śr. punkty", reset: "Reset",
    ...
    calculatingSuggestion: "Obliczanie najlepszego ruchu…",
    noMovesPossible: "Brak ruchów — gra się skończyła.",
    ...
    cardInPlay: "Ta karta jest już w grze.",
    deckEmptyNoThrow: "Nie możesz odrzucić — talia jest pusta, nic nie dojdzie.",
    pickScoresNothing: "Te 3 karty nie dają punktów — nie zostały wzięte.",
